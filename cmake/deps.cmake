	...
	if(NOT TARGET ${name})
		...
		if(TARGET ${this_target})
			if(${name}_INCLUDE_DIR)
				target_include_directories(${this_target} ${visibility} ${${name}_INCLUDE_DIR})
			endif()
			if(${name}_INCLUDE_DIRS)
				target_include_directories(${this_target} ${visibility} ${${name}_INCLUDE_DIRS})
			endif()
			target_link_directories(${this_target} ${visibility}
...
macro(dep)
	...
	foreach(dep ${ARGN})
		...
		if(NOT TARGET ${dep} AND TARGET ${dep}::${dep})
			set(dep ${dep}::${dep})
		endif()
		...
		if(TARGET ${dep})
			...
			if(${type} MATCHES "STATIC_LIBRARY|MODULE_LIBRARY|SHARED_LIBRARY|UNKNOWN_LIBRARY")
				message("'${dep}'  MATCHES STATIC_LIBRARY|MODULE_LIBRARY|SHARED_LIBRARY|UNKNOWN_LIBRARY")
				target_link_libraries(${this_target} ${visibility} ${dep})
			else()
		...
		elseif(${dep} STREQUAL pthread)
        	target_link_options(${this_target} ${visibility} -pthread)
...
macro(deps)
    ...
    foreach(tg ${dependencies_listified})
		...
		elseif(${tg}_LIBRARIES)
			list(APPEND this_target_libs ${${tg}_LIBRARIES})
		else()
    ...
    message("target_link_directories(${this_target} ${visibility} ${CMAKE_CURRENT_BINARY_DIR})")
    target_link_directories(${this_target} ${visibility} ${CMAKE_CURRENT_BINARY_DIR})
    if(this_target_libs AND NOT this_target_libs STREQUAL "")
		...
		foreach(lib ${this_target_libs})
			...
			if(NOT lib STREQUAL "optimized"
				...
				message("lib: ${lib}")
				target_link_libraries(${this_target} ${visibility} ${lib})
			endif()
		endforeach()
    else()
    ...
    if(NOT MSVC)
        target_link_options(${this_target} ${visibility} -pthread)
		if(NOT APPLE)
	        target_link_libraries(${this_target} ${visibility} ${CMAKE_DL_LIBS})
		endif()
    endif()
