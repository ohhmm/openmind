...
macro(glob_source_files)
	...
	if(IS_DIRECTORY ${CMAKE_CURRENT_SOURCE_DIR}/${platform})
		file(GLOB platform_specific_headers ${platform}/*.h ${platform}/*.hpp ${platform}/*.inc ${platform}/*.hxx)
		list(APPEND headers ${platform_specific_headers})

		file(GLOB platform_specific_src ${platform}/*.cpp ${platform}/*.c ${platform}/*.CC ${platform}/*.cc)
		list(APPEND src ${platform_specific_src})

		file(GLOB platform_specific_qmlsrc ${platform}/*.qml)
		list(APPEND qmlsrc ${platform_specific_qmlsrc})
...
macro(exe)
    ...
    elseif(ANDROID)
		...
		add_executable(${this_target} ${src} ${resources})
    endif()
    APPLY_TARGET_COMMONS(${this_target})
    #add_dependencies(${this_target} prerequisites)
    if(Qt${QT_VERSION_MAJOR}_FOUND)
