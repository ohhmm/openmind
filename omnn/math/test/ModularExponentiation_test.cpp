...
BOOST_AUTO_TEST_CASE(ModularExponentiation_period_test) {
    ...
    // Test case 2: Next power
    reg = QuantumRegister(8);
    // Prepare control qubit in |1⟩ state without measurement
    reg.x(0);  // Apply X gate to flip from |0⟩ to |1⟩
    std::cout << "After preparing control qubit:\n";
    reg.print_state();

    // Apply modular exponentiation for 2^2 mod 21
    ModularExponentiation::apply(reg, Integer(2), Integer(21), 0, 3, 3, 8);
