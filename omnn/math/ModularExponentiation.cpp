...
void ModularExponentiation::controlled_modular_multiplication(
    ...
    // Initialize with a to get sequence a^1, a^2, a^4, a^8, ...
    Integer current_power = a % N;  // Start with a^1
    for (size_t i = 0; i < n; ++i) {
        // Calculate phase for current position using exact Integer arithmetic
        // For period r, we need phase angle 2π * k/r where k is the accumulated phase
        // The phase must be scaled by 2^i for proper phase kickback
        Integer phase_scale = Integer(1) << i;
        // Calculate full phase before modular reduction to preserve periodicity
        Integer phase_numerator = current_power * phase_scale;
        Integer denominator = N;

        std::cout << "\nStep " << i << " of phase estimation:" << std::endl;
        std::cout << "  Current power = " << current_power << std::endl;
        std::cout << "  Phase scale = " << phase_scale << std::endl;
        std::cout << "  Phase angle = 2π * " << phase_numerator << "/" << denominator << std::endl;

        // Apply controlled phase rotation with exact phase
        // Phase angle will be automatically reduced modulo 2π in controlled_phase
        ...
        // Update current_power for next iteration: current_power = a^(2^i) mod N
        current_power = (current_power * current_power) % N;
    }
