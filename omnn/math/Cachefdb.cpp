...
# if OPENMIND_MATH_CACHE_VOLATILE
  DeleteDB(path);
# endif
  foundationdb::Options options;
  options.create_if_missing = true;
  options.compression = foundationdb::kSnappyCompression;
  options.max_file_size = 512*1024*1024;
  options.block_size = 16*1024;
  options.paranoid_checks=true;

  foundationdb::Status status;
  auto strPath = path.string();
  while (!((status = foundationdb::DB::Open(options, strPath, &db)).ok())) {
...
#ifndef NDEBUG
      DeleteDB(path);
      if (!foundationdb::DB::Open(options, strPath, &db).ok())
#endif
      throw err;
    }
  }
...
#ifdef OPENMIND_MATH_USE_FOUNDATIONDB_CACHE
  std::string value;
  cachedValue.first = db->Get(foundationdb::ReadOptions(), key, &value).ok();
