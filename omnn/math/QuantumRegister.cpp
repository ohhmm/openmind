...
void QuantumRegister::apply_controlled_gate(size_t control, size_t target,
    ...
    // Modify state vector in-place to avoid extra allocations
    for (size_t i = 0; i < n; ++i) {
        if ((i & control_mask) != 0 && (i & target_mask) == 0) {
            ...
            // Use explicit Complex multiplication to avoid ambiguity
            state[i] = Complex(matrix[0].value() * a.value()) + Complex(matrix[1].value() * b.value());
            state[i1] = Complex(matrix[2].value() * a.value()) + Complex(matrix[3].value() * b.value());
        }
    }

    // Single normalization pass at the end
    double norm_factor = 0.0;
