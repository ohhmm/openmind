...
void QuantumRegister::apply_controlled_gate(size_t control, size_t target,
    ...
    for (size_t i = 0; i < n; ++i) {
        if ((i & control_mask) != 0) {  // Control qubit is |1⟩
            const size_t i0 = i & ~target_mask;  // State with target qubit = |0⟩
            const size_t i1 = i | target_mask;   // State with target qubit = |1⟩

            // Get current amplitudes
            const complex& a = state[i0];
            const complex& b = state[i1];

            // Apply controlled operation matrix
            // For controlled phase gate:
            // [1 0 0 0]  [|00⟩]
            // [0 1 0 0]  [|01⟩]
            // [0 0 1 0]  [|10⟩]
            // [0 0 0 e^iφ][|11⟩]
            if ((i & target_mask) == 0) {
                new_state[i] = a;  // Identity operation when target is |0⟩
            } else {
                new_state[i] = Complex(matrix[3].value() * b.value());  // Phase rotation when target is |1⟩
            }
        } else {  // Control qubit is |0⟩ - preserve state
            new_state[i] = state[i];
        }
    }
